...
class LLDPSender (object):
  ...
  def addSwitch (self, dpid, ports):
	...
	for portNum, portAddr in ports:
	  ...
	  self._packets.append(LLDPSender.SendItem(dpid, portNum,
	   self.create_discovery_packet(dpid, portNum, portAddr)))			# Append the new packet to send the request

	self._setTimer()	#start the timer considering the number of packets
  ...
  def _timerHandler (self):
	...
	core.openflow.sendToDPID(item.dpid, item.packet)						#send to the sw the packet to be broadcast
...
class Discovery (EventMixin):
  ...
  Sinfo = namedtuple("Sinfo",("dpid","port","hardw"))
  ...
  def __init__ (self, install_flow = True, explicit_drop = True):
	...
	self.swinfo = {}	# For store the switches information
  ...
  def _handle_PacketIn (self, event):
	...
	#print 'LLDP packet in: %d (%s) -> %d (%s)' % (originatorDPID,str(originatorPort), event.dpid, str(event.port))

	link = Discovery.Link(originatorDPID, originatorPort, event.dpid, event.port)

	if link not in self.adjacency:
	  self.adjacency[link] = time.time()
	  #log.info('link detected: %s.%i -> %s.%i' % (dpidToStr(link.dpid1), link.port1, dpidToStr(link.dpid2), link.port2))
	  self.raiseEventNoErrors(LinkEvent, True, link)			#LC: the spanning_tree.py listen for this event
	else:
  ...
  def _deleteLinks (self, links):
	for link in links:
	  ...
	  self.raiseEvent(LinkEvent, False, link)			#LC: the spanning_tree.py listen for this event
...
def launch (explicit_drop = False, install_flow = True):
	...
	core.registerNew(Discovery, explicit_drop=explicit_drop, install_flow=install_flow)
